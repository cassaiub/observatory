...
\thead{What} & \thead{Why} \\
\midrule
A laptop running Linux or macOS & One install route for both. Windows works too, but only
through WSL --- there is no native Windows install, so start that part early. \\
...
Phase 2 plate-solves your images against a star catalog, and the solver is a
compiled binary rather than a Python package. Miniforge is what supplies it,
along with prebuilt versions of the whole scientific stack, so nothing has to
compile on your laptop.
\end{note}

\subsection{Windows only: get to Linux first}
There is no native Windows install. No plate solver is published for Windows ---
not on conda-forge, not on PyPI --- so a native environment would build and then
fail at the first WCS solve, leaving you with no zero point and no calibrated
magnitude. WSL gives you real x86-64 Linux, where everything works.

From an \textbf{administrator} PowerShell:
...
Reboot when it asks, open \textbf{Ubuntu} from the Start menu, and let it finish
creating your user account. Everything from here on happens inside that Ubuntu
window, and every command is the Linux one.

\begin{caution}[Keep your files on the Linux side]
Your Windows drives appear under \cm{/mnt/c}, so data you downloaded in Windows
is reachable. But keep the repository and the work directory in your WSL home
--- reducing a night across \cm{/mnt} is several times slower.
\end{caution}
...
\cm{cassa-doctor} is the one command that proves the install: it reports the
package version, the environment, the plate solver and the index directory.
...
\begin{note}[Which solver you get]
On x86-64 Linux and Intel macOS the script installs Astrometry.net's
\cm{solve-field}. On Apple Silicon there is no build of it, so the in-process
Python solver is used instead --- slower on hard fields, but correct, and
nothing else in the session changes. \cm{cassa-doctor} tells you which one you
have.
\end{note}
...
\cm{solve-field: NOT FOUND} &
Expected on a pip install. The in-process solver is used instead. Only a problem if Phase 2
fails to solve. \\
...
Dhaka.\\
Pipeline documentation lives in the repository: \texttt{README.md} and
\texttt{docs/CUSTOMIZING.md}.}
