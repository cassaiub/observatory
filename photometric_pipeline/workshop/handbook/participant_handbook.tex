...
Windows                  & WSL, then the Linux route & Do the WSL part first; it needs a reboot. \\
\bottomrule
\end{tabularx}
...
route that is actually tested, which is why it is the one documented.
...
\begin{note}[Running \cm{install.ps1} does nothing]
It installs nothing on purpose. It prints these WSL steps and stops, so a
Windows participant is not left with an environment that builds and then cannot
solve. \cm{cassa-doctor} reports native Windows the same way.
\end{note}
