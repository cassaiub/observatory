...
Windows                  & WSL, then the Linux route & \textbf{Use this for the session.} Do it first; it needs a reboot. \\
\bottomrule
\end{tabularx}

\subsection{Windows only: get to Linux first}
The pipeline is developed and tested on Linux and macOS, and \cm{install.sh} is
a shell script. WSL gives you real x86-64 Linux on your Windows machine, so
every instruction in this handbook then applies unchanged --- and it is the
route that is actually tested, which is why it is the one to use for the
session.
...
\begin{note}[There is now a native Windows installer, but not for today]
\cm{.\textbackslash install.ps1} does build a real native environment: ASTAP
publishes a Windows plate solver and every dependency has a Windows wheel. It
has not yet been verified end to end, though, so \cm{cassa-doctor} calls it
provisional --- and a workshop is the wrong place to be the first person to try
it. Use WSL today. If you want to test the native route afterwards, the result
either way is genuinely useful to report.
\end{note}
